...
export default function AudienceSection() {
  ...
  return (
    <section className="bg-[#0A0A0F] py-24 md:py-36">
      <div className="mx-auto max-w-7xl px-6 md:px-10">
        
        {/* Editorial Heading Design */}
        <div className="mb-16 text-center max-w-3xl mx-auto space-y-4">
          <h2 className="font-display text-4xl font-extrabold tracking-tight text-paper sm:text-5xl md:text-6xl leading-[1.05]">
            Real people <br className="sm:hidden" />
            building <span className="italic font-normal font-serif text-signal">real things.</span>
          </h2>
          <p className="text-base text-paper/60 sm:text-lg">
            A verified professional network defined by <strong className="font-semibold text-paper">relevance</strong>, not social algorithms.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
          {cards.map((c) => (
            <div
              key={c.label}
              className="group animate-audience-card opacity-0 relative overflow-hidden rounded-[2rem] bg-[#0F172A] shadow-lg transition duration-500 hover:shadow-xl"
            >
              {/* Photo Showcase */}
              <div className="relative aspect-[3/4.2] w-full overflow-hidden">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-[#080B11]/95 via-[#080B11]/35 to-transparent transition-opacity duration-300" />
              </div>
              
              {/* Glassmorphic Overlay Text Card */}
              <div className="absolute bottom-4 left-4 right-4 p-5 rounded-2xl border border-white/10 bg-[#080B11]/60 backdrop-blur-md transition-transform duration-500 group-hover:translate-y-[-2px]">
